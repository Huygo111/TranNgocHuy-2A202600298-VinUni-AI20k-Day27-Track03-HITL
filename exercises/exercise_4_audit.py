...
from common.db import checkpoint_db_path, write_audit_event
        ...
        a: PRAnalysis = await llm.ainvoke([
            {
                ...
                "content": (
                    ...
                    "specific questions tied to the diff."
                ),
            },
...
async def node_route(state):
    ...
    await audit(state, AuditEntry(
        ...
        decision="auto" if decision == "auto_approve" else decision,
...
def _post(state) -> str:
    try:
        ...
        console.print(f"  [red]X[/red] post failed: {e}")
...
async def node_commit(state):
    ...
    if state.get("escalation_answers"):
        reason = "Committed refined review after escalation"
    elif action == "rejected":
        reason = reason or "Reviewer rejected the automated review"
    ...
    await audit(state, AuditEntry(
        ...
        decision="approve" if action == "committed" else "reject" if action == "rejected" else action,
        reason=reason,
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
...
async def node_escalate(state):
    ...
    await audit(state, AuditEntry(
        ...
        risk_level=risk_level_for(a.confidence),
        decision="escalate",
        reason="Escalation requested: " + "; ".join(questions),
        execution_time_ms=int((time.monotonic() - t0) * 1000),
    ))
        ...
        refined: PRAnalysis = await llm.ainvoke([
            {
                "role": "system",
                "content": (
                    "Refine the earlier structured PR review using the reviewer's answers. "
                    "Return updated confidence, summary, comments, and risk factors."
                ),
            },
            {
                "role": "user",
                "content": (
                    ...
                    f"Diff:\n{state['pr_diff']}\n\n"
                    f"Original confidence reasoning:\n{state['analysis'].confidence_reasoning}\n\n"
                    f"Q&A:\n{qa}"
                ),
            },
        ])
